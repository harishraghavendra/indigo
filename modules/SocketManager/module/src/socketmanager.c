...
#define IND_SOC_RUN_IS_EXITING()     (ind_soc_run_status__ == IND_SOC_RUN_STATUS_EXIT)
...
process_timers(ind_soc_priority_t priority)
{
    ...
    while (!list_empty(&tmp_list)) {
        ...
        if (IND_SOC_RUN_IS_EXITING() || (timer->priority != priority)) {
...
process_tasks(ind_soc_priority_t priority)
{
    ...
    LIST_FOREACH_SAFE(&tasks, cur, next) {
        ...
        if (IND_SOC_RUN_IS_EXITING() || (task->priority < priority)) {
            break;
        }
